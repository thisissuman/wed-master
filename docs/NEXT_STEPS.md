...
Current design baseline: the warm ivory/lavender implementation with selective deep-plum night
surfaces and restrained code-native ornament. Legacy green references and retired full-screen
heart artwork are historical only and must not be used as active design input.
...
| LB-05 | P1       | Pending | Physical Android accessibility/device pass | 360dp, expanded/tablet, portrait/landscape, largest text, TalkBack, keyboard/IME, reduced motion, share picker, permission denial, predictive/hardware Back, process termination, and upgrade install verified                                                         | Current source passes emulator portrait, 914dp landscape rail, and 1.3×/2.0× text; physical Android phone/tablet access is still required | 2026-08-13 |
| LB-06 | P1       | Blocked | Preview distribution and observability     | Final preview APK identifies as `com.suman.mangalya.preview`; signing, deep links, launcher/splash, redesigned rail, scrubbed Sentry event, and source maps verified                                                                                                   | Build 4 is signed and inspected but predates the redesign; wait for one final batched APK, and Sentry credentials remain unavailable      | 2026-08-13 |
...
| Preview APK               | `npx eas-cli build --platform android --profile preview`                                                                                                                             | Signed installable APK reports `com.suman.mangalya.preview` and `mangalya-preview` without unnecessary permissions | Build 4 `57874f60-9ec5-44c2-9bfe-eef8f54e02fe` is signed, inspected, and installed, but predates the redesign. Produce one final batched APK after source completion. |
